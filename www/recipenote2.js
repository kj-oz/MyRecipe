...
$(document).ready(function() {
  ...
  var init = function () {
    ...
    if (!version) {
      msg = "これまで複数の端末でデータを共有するためにDropboxを利用していましたが、" + 
            "ブラウザやDropboxのバージョンアップに伴い正常に動作しなくなってしまったため、" + 
            "本バージョンからデータを独自のサーバーに保存するようになりました。\n" + 
            "このバージョンを初めてお使いいただく際に、一度ユーザー登録を行っていただく必要があります。" + 
            "（登録するユーザーをDropboxと合わせる必要はありません）\n" + 
            "なお、各端末が持っている既存のデータはそのままお使いいただけます。\n" + 
            "これまでDropboxにログインせずにお使いの場合は、ログイン画面でキャンセルボタンを" +
            "タップすることで今後も端末単独でお使いいただくことが出来ます。";
      alert(msg);
