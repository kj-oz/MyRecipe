...
KOJS.recipe.model.Repository = (function () {
   ...
   */
  Repository = function (serverPath) {
    ...
    this._dao = new KOJS.recipe.model.Dao();
  };
  ...
  Repository.prototype = {
    ...
    uploadAll: function () {
      ...
      self._dao.authenticate().then(function () {
        ...
        for (i = 0, n = recipes.length; i < n; i++) {
        ...
        for (ruby in self._dictionary) {
          if (self._dictionary.hasOwnProperty(ruby)) {
            rubies.push(ruby);
          }
        }
        for (i = 0, n = rubies.length; i < n; i++) {
          (function (ruby) {
            funcs.push(self._dao.postDict(ruby, self._dictionary[ruby]));
          })(rubies[i]);
        }

        $.when.apply(null, funcs).always(function () {
          logger.log("upload all done.");
          deferred.resolve();
        });
      }, function () {
        deferred.reject();
    ...
    syncWithServer: function () {
      ...
      return self._dao.authenticate().then(function () {
        ...
        var lastDownload = Number(localStorage["lastDownload"] || 0),
            id, recipe,
        ...
        self._dao.loadRecipes(lastDownload).then(function (recipes) {
          serverUpdates.recipes = recipes.filter(
          ...
          return self._dao.loadDicts(lastDownload).then(function (dicts) {
            ...
            for (i = 0, n = dicts.length; i < n; i++) {
              data = dicts[i];
              self._dictionary[data.ruby] = data.words;
              for (j = 0, m = data.words.length; j < m; j++) {
                self._words[data.words[j]] = data.ruby;
              }
            }
            self.saveDictionary();
              ...
              funcs.push(self._dao.postRecipe(recipe).then(function (newId) {
                ...
                return self._checkNewWords(recipe).then(function (data) {
                  ...
                  for (i = 0, n = data.length; i < n; i++) {
                    jobWords.push(
                          self._updateDictionary(data[i].ruby, data[i].word));
                  }
                }, function () {
          ...
          for (word in self._newWords) {
            if (self._newWords.hasOwnProperty(word)) {
              jobWords.push(self._updateDictionary(
                      self._newWords[word], word).done(function () {
        ...
        logger.log("authenticate fail: " + error);
      });
    },
     ...
     * @param {Boolean} readFirst 最初にサーバーから与えられた読みに対する最新の情報を
     *                  読み込むかどうか
     * @return {Promise} 処理に対するPromiseオブジェクト
     */
    _updateDictionary: function (ruby, word, readFirst) {
      ...
      logger.log("- updateDictionary start: " + ruby + " " + word);
      if (readFirst) {
        self._dao.getDict(ruby).then(function (dict) {
          logger.log("- updateDictionary getDict done: " + ruby + " " + word);
          var i, n;
          if (dict) {
            self._dictionary[dict.ruby] = dict.words;
            for (i = 0, n = dict.words.length; i < n; i++) {
              self._words[dict.words[i]] = dict.ruby;
            }
          }
          if (!self._words[word]) {
            self._addWord(ruby, word, true);
          }
          deferred.resolve();
        }, function () {
          self._addWord(ruby, word);
          logger.log("- updateDictionary getDict fail: " + ruby + " " + word);
          deferred.resolve();
        });
        return deferred.promise();
      } else {
        logger.log("- updateDictionary addWord: " + ruby + " " + word);
    ...
    _addWord: function (ruby, word, post) {
      ...
      if (post) {
        self._dao.postDict(ruby, self._dictionary[ruby]).then(function () {
          logger.log("- addWord post done: " + ruby + " " + word);
          deferred.resolve();
        }, function () {
          logger.log("- addWord post fail: " + ruby + " " + word);
          self._newWords[word] = ruby;
          deferred.resolve();
        });
      } else {
        logger.log("- addWord not post: " + ruby + " " + word);
        self._newWords[word] = ruby;
        deferred.resolve();
      }
      return deferred.promise();
    ...
    saveRecipes: function () {
      try {
        ...
        alert("これ以上レシピを追加することが出来ません。不要なレシピを削除してださい。")
      }   
    },
