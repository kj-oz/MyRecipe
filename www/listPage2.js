  ...
  var ListPage = {
    ...
    setup: function (context) {
      ...
      $("#list-page").on("pagebeforecreate", function (event) {
        ...
        $("#ls-admin-login").on("click", function(e) {
          logger.log("> admin-login tapped");
          KOJS.recipe.page.LoginPage.show("#list-page").then(function() {
            localStorage.removeItem("offline");
            document.location = "recipenote2.html"
          });
        });
        
        $("#ls-admin-logout").on("click", function(e) {
          logger.log("> admin-logout tapped");
          KiiUser.logOut();
          localStorage["offline"] = true;
          self._updateLoginStatus();
        });

        $("#ls-admin-changepassword").on("click", function(e) {
          logger.log("> admin-changepassword tapped");
          $.mobile.changePage("#changepassword-page", 
        ...
        $("#ls-recipes").on("click", "a", function(e) {
          ...
          if ($a.text() === "レシピ一覧") {
            if (context.repository.getCount() >= 1000) {
              alert("これ以上レシピを追加することが出来ません。不要なレシピを削除してからお試しください。")
            } else {
...
//        }
      ...
      this._updateLoginStatus = function () {
        if (KiiUser.getCurrentUser()) {
          $("#ls-admin-init").removeClass("ui-disabled");
          $("#ls-admin-update").removeClass("ui-disabled");
          $("#ls-admin-login").addClass("ui-disabled");
          $("#ls-admin-logout").removeClass("ui-disabled");
          $("#ls-admin-changepassword").removeClass("ui-disabled");          
        } else {
          $("#ls-admin-init").addClass("ui-disabled");
          $("#ls-admin-update").addClass("ui-disabled");
          $("#ls-admin-login").removeClass("ui-disabled");
          $("#ls-admin-logout").addClass("ui-disabled");
          $("#ls-admin-changepassword").addClass("ui-disabled");          
        }
       };
    }
  }
